...
def create_data_structure(string_input):
    ...
    information = string_input.split('.')
    while n <len(information):
        connects = information[n-1]
        games = information[n]
        net = []
        connect = []
        game = []
        con_start = connects.find('to')
        name = connects.split()[0]
        con_list = connects[con_start + 2:]
        con_list = con_list.split(',')
        for i in con_list:
            i = i.strip()
            connect.append(i)
        game_start = games.find('play')
        game_list = games[game_start+4:]
...
def get_secondary_connections(network, user):
    if user not in network:
        return None
    net_list = []
    for items in network[user][0]:
        for item in network[items][0]:
            if item not in net_list:
...
def count_common_connections(network, user_A, user_B):
    list = 0
    if user_A in network and user_B in network:
        for item in network[user_A][0]:
            if item in network[user_B][0]:
                list +=1
        return list
    return False
